...
char		**get_filenames(int total, char **argv) {
  char	**filenames = NULL;
  int	i = 0;

  printf("DEBUG: total %d\n", total);
  if ((filenames = malloc((sizeof(*filenames) * (total + 1)))) != NULL) {
    while (i < total) {
      filenames[i] = argv[i + 6];
      puts(filenames[i]);
      i++;
    }
    filenames[i] = NULL;
...
void run_detector(int argc, char **argv)
{
  ...
  if (argc > 6) {
    ...
    filenames = get_filenames(argc - 8, argv);
  }
  ...
  if(0==strcmp(argv[2], "test")) test_detector(datacfg, cfg, weights, filename, thresh, dont_show);
    ...
    demo(cfg, weights, thresh, cam_index, filenames, names, classes, frame_skip, prefix, out_filename, http_stream_port, dont_show);
